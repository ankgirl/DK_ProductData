...
- **진단·임시 스크립트·분석 결과·일회성 데이터 파일은 `_scratch/` 폴더 안에만 작성.**
  - `_scratch/`는 `.gitignore`에 등록되어 절대 커밋되지 않음
  - 메인 코드(`public/`, 프로젝트 루트)에 진단/임시 파일을 두지 말 것 — 합쳐 두면 폐기 누락·실수 커밋 위험
  - 인라인 실행 결과로 만든 텍스트 리포트·xlsx 등도 모두 `_scratch/` 아래로 저장
- 운영 DB 접근 진단 스크립트(REST PATCH/GET 등)도 모두 `_scratch/`에서 실행하고 산출물 보관
